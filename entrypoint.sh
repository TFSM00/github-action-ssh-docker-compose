...
remote_command="set -e ; log() { echo '>> [remote]' \$@ ; } ; cleanup() { log 'Removing workspace...'; rm -rf \"\$HOME/microtasker\" ; }; cleanup ; log 'Creating workspace directory...' ; mkdir -p \"\$HOME/microtasker\"; log 'Unpacking workspace...' ; tar -C \"\$HOME/microtasker\" -xjv ; log 'Launching docker-compose...' ; cd \"\$HOME/microtasker\" ; cp \"\$HOME/microtasker-cfg/env-vars.txt\" ./ ; docker compose -f \"$DOCKER_COMPOSE_FILENAME\" -p \"$DOCKER_COMPOSE_PREFIX\" pull ; docker compose -f \"$DOCKER_COMPOSE_FILENAME\" -p \"$DOCKER_COMPOSE_PREFIX\" up -d --remove-orphans --build"
