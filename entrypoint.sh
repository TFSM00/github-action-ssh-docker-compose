...
if $DOCKER_COMPOSE_DOWN ; then
  remote_command="set -e ; log() { echo '>> [remote]' \$@ ; } ; cleanup() { log 'Removing workspace...'; rm -rf \"\$HOME/microtasker\" ; } ; log 'Creating workspace directory...' ; mkdir -p \"\$HOME/microtasker\" ; trap cleanup EXIT ; log 'Unpacking workspace...' ; tar -C \"\$HOME/microtasker\" -xjv ; log 'Launching docker-compose...' ; cd \"\$HOME/microtasker\" ; cp \"\$HOME/microtasker-cfg/env-vars.txt\" ./ ; docker compose --sysctl net.ipv6.conf.all.disable_ipv6=1 -f \"$DOCKER_COMPOSE_FILENAME\" -p \"$DOCKER_COMPOSE_PREFIX\" down"
fi
